...
// Skip the whole init when no DSN is configured: an eagerly-initialised SDK
// still installs its integrations (fetch/XHR/history patching, router
// instrumentation) and does nothing useful without a DSN.
if (SENTRY_DSN) {
  Sentry.init({
    dsn: SENTRY_DSN,
