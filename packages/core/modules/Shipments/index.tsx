...
// One row of the board, as GET_SHIPMENTS_LIST returns it.
type ListShipment = get_shipments_list_shipments_edges_node;
type ListRate =
  | ListShipment["rates"][number]
  | NonNullable<ListShipment["selected_rate"]>;
// A carrier connection as the two connection hooks hand it over: only the
// three fields the row reads (matching id / carrier_id, avatar colours).
type ConnectionLike = {
  id?: string | null;
  carrier_id?: string | null;
  config?: { text_color?: string; brand_color?: string } | null;
};

// How many ERP-relayed rows a bulk run keeps in flight. The Karrio API has
// two request slots in production (gunicorn, 2 sync workers) and every ERP
// relay call holds one for its whole round trip, so more than this only
...
const erpLockKey = (shipment: Pick<ListShipment, "id" | "metadata">) => {
  const values = (shipment.metadata || {}) as Record<string, unknown>;
  const key = values["sales_order"] ?? values[SHOPIFY_ORDER_NUMBER_KEY];
  return typeof key === "string" || typeof key === "number"
    ? `${key}`
    : shipment.id;
};

const getRate = (shipment: ListShipment): ListRate | ListShipment =>
  shipment.selected_rate ||
  (shipment.rates || []).find(
    (rate) => rate.service === shipment?.options?.preferred_service,
  ) ||
  (shipment.rates || [])[0] ||
  shipment;
const getCarrier = (
  rate: ListRate | ListShipment,
...
const renderColli = (shipment: Pick<ListShipment, "parcels">) => {
...
const renderPrintOverdue = (metadata: unknown) => {
  ...
  if (!printDate) return <p className="text-xs font-medium text-gray-400">—</p>;
...
// One table row. Memoized: at page size 100 the board re-renders on every
// selection click, filter change and refetch, and a row's inputs (its
// node — structurally shared by react-query across refetches — and the
// view flags) rarely change. The handlers are stable callbacks (refs
// underneath) so the memo actually holds.
const ShipmentRow = React.memo(function ShipmentRow({
  ...
  const route = routeLabel(shipment.metadata);
  const carrier =
...
// Module scope, not a closure inside Page: a component type created per
// Page render is a new element type each time, which remounts the whole
...
function ShipmentsBoard(): JSX.Element {
  ...
  // Both silent: every mutation here runs inside a bulk loop, and the loop
  // refreshes the board once when it is done (invalidateShipmentsAroundMirror)
  // instead of once per row.
  const mutation = useShipmentMutation(undefined, { silent: true });
  ...
  const context = useShipments({
    ...
    variant: "list",
    // The Failed card renders FailedShipmentsList instead of this list;
    // fetching a page of every other status behind it is wasted.
    isDisabled: (filter: ShipmentFilter) =>
      ([] as string[])
        .concat((filter.status as any) || [])
        .includes(FAILED_SENTINEL),
  });
  const {
  ...
  const updatedSelection = (
    ...
    const selected =
      selection.length > 0 && selection.length === (shipment_ids || []).length;
  ...
  // The row handlers read the latest selection/visible rows through refs
  // so they can be stable callbacks — a new handler per render would
  // defeat React.memo on every row.
  const selectionRef = React.useRef(selection);
  selectionRef.current = selection;
  const visibleRef = React.useRef<{ node: ListShipment }[]>([]);
  const handleCheckboxChange = React.useCallback(
    (checked: boolean, name: string) => {
      if (name === "all") {
        lastCheckedRef.current = null;
        setSelection(
          !checked ? [] : visibleRef.current.map(({ node: { id } }) => id),
        );
      } else {
  ...
  const handleSelectorClick = React.useCallback(
    (event: React.MouseEvent, id: string) => {
      ...
      const ids = visibleRef.current.map(({ node: shipment }) => shipment.id);
  ...
  const runMarkOutForDelivery = async () => {
    ...
    await runBounded(
      ...
      async (shipment) => {
        try {
          await erpActions.markOutForDelivery.mutateAsync({
            id: shipment.id,
          });
          succeeded += 1;
        } catch (error) {
  ...
  // confirmation; same bounded loop and per-row error collection.
  ...
  // Same bounded loop and per-row error collection as the other runners.
    ...
    await runBounded(
      ...
      async (shipment) => {
        try {
          ...
          // A tracked shipment's status belongs to the carrier; the
          // operator may override it only with a reason (recorded
          // server-side onto metadata.status_override for the audit). The
          // reason from the dialog is passed along, so the override is
          // allowed — but a refusal here must never fail the row: the ERP
          // already has the outcome on the record, and reporting it as
          // lost would be a lie.
          try {
  ...
  const selectOutcome = (option: OutcomeOption) =>
    option.requiresReason ? setOutcomePrompt(option) : runRecordOutcome(option);

  // useShipments seeds its filter from the URL, so the params present at
  // mount are already applied and need no second fetch. Only a LATER
  // change of the URL (back/forward, a link opened in this tab) has to be
  // pushed into the filter; our own setFilter → pushState round trip
  // yields an identical filter and therefore no refetch.
  const searchParamsString = searchParams?.toString() ?? "";
  const appliedSearchParams = React.useRef(searchParamsString);
  useEffect(() => {
    if (appliedSearchParams.current === searchParamsString) return;
    appliedSearchParams.current = searchParamsString;
    updateFilter();
  }, [searchParamsString]);
  useEffect(() => {
                        ...
                        {isTodayView &&
                          isEnabled("btn_buy_label_dashboard") && (
                            <Button
                              ...
                              size="sm"
                              disabled={
                                bulkAction !== null || documentPrinter.isLoading
                              }
                              className="px-3"
