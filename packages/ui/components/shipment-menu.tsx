...
// The trigger renders for every row; the menu body — and with it the
// ~21 mutation observers of useShipmentMutation + useShipmentERPActions,
// the template query and the feature flags — mounts on the first open and
// then stays mounted, so the confirmation/reason dialogs it owns survive
// the menu closing (a click on an item closes the menu before the dialog
// opens). At page size 100 that is 2 100 observers the board no longer
// creates on every render for menus nobody opened.
export const ShipmentMenu = ({
  ...
  return (
    <div key={`menu-${shipment.id}`}>
      <DropdownMenu onOpenChange={(open) => open && setMounted(true)}>
        ...
        {mounted && (
          <ShipmentMenuContent shipment={shipment} isViewing={isViewing} />
        )}
      </DropdownMenu>
    </div>
  );
...
const ShipmentMenuContent = ({
  shipment,
  isViewing,
}: Pick<ShipmentMenuComponent, "shipment" | "isViewing">): JSX.Element => {
  ...
  const erpDraft = erpLinked && shipment.status === ShipmentStatusEnum.draft;
  ...
  const buyAndPrintLabel = async () => {
    ...
    try {
      ...
      await mutation.buyLabel.mutateAsync({
        ...shipment,
        id: shipment.id,
        selected_rate_id: shipment.selected_rate?.id ?? shipment.rates?.[0]?.id,
      } as ShipmentType);
      toast({
  ...
  return (
    <>
      ...
      >
        ...
        {erpLinked &&
          erpStatus === "Label Created" &&
          isEnabled("btn_mark_shipped") && (
            <DropdownMenuItem
              onClick={markShipped}
              disabled={
      ...
      {confirmAction && (
        <DeleteConfirmationDialog
          ...
          confirmLabel={
            confirmAction.title === "Cancel Shipment"
              ? "Cancel"
              : confirmAction.confirmLabel
          }
          cancelLabel={
            confirmAction.title === "Cancel Shipment" ? "Go Back" : "Cancel"
          }
